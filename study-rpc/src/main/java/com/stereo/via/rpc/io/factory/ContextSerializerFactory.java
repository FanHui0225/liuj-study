...
import com.stereo.via.rpc.io.deserializer.*;
import com.stereo.via.rpc.io.serializer.*;
...
public class ContextSerializerFactory {
	private static final Logger log = Logger
			.getLogger(ContextSerializerFactory.class.getName());
	...
	public Serializer getCustomSerializer(Class cl) {
		...
		try {
			Class serClass = Class.forName(cl.getName() + "Serializer", false,
					cl.getClassLoader());
	...
	public Deserializer getCustomDeserializer(Class cl) {
		...
		if (deserializer == AbstractDeserializer.NULL)
			...
			return deserializer;
		try {
			Class serClass = Class.forName(cl.getName() + "Deserializer",
	...
	private void init() {
		...
		initSerializerFiles("META-INF/rpc/serializers", _serializerFiles,
				classMap, Serializer.class);
		...
		classMap = new HashMap<Class, Class>();
		initSerializerFiles("META-INF/rpc/deserializers", _deserializerFiles,
				classMap, Deserializer.class);

		for (Map.Entry<Class, Class> entry : classMap.entrySet()) {
			try {
				Deserializer ser = (Deserializer) entry.getValue()
						.newInstance();
			...
			while (iter.hasMoreElements()) {
				...
				try {
					...
					for (Map.Entry entry : props.entrySet()) {
						...
						try {
							apiClass = Class.forName(apiName, false,
									classLoader);
						} catch (ClassNotFoundException e) {
							log.fine(url + ": " + apiName
						...
						if (!type.isAssignableFrom(serializerClass))
							throw new RpcRuntimeException(
									url
											...
											+ " is invalid because it does not implement "
											+ type.getName());
	...
	static {
		...
		_staticSerializerMap.put(byte[].class.getName(),
				ByteArraySerializer.SER);
		...
		_staticDeserializerMap.put(Number.class.getName(),
				new BasicDeserializer(BasicSerializer.NUMBER));
