...
import com.stereo.study.rpc.exc.ServiceException;
 ...
 * @author stereo
 */
public class Input extends AbstractInput implements Constants {
	...
	// }
	// if (tag != 'E')
	// throw error("expected rpc Envelope ('E') at " + codeName(tag));
	// return version;
	// }
	...
	public Object readReply(Class expectedClass) throws Throwable {
		...
		if (tag == 'R')
			...
			throw error("expected rpc reply at " + codeName(tag) + "\n" + sb);
		}
	}
	...
	public int startMessage() throws IOException {
		...
		if (tag == 'p') {
			...
			throw error("expected rpc message ('p') at " + codeName(tag));
	...
	public Object readObject(Class cl) throws IOException {
		...
		case 0x6f: {
			...
			if (ref < 0 || size <= ref)
				throw new ProtocolException("'" + ref
	...
	public void resetBuffer() {
		...
		if (length > 0 && offset != length)
			throw new IllegalStateException("offset=" + offset + " length="
					+ length);
	}
	...
	class ReadInputStream extends InputStream {
		...
		public int read(byte[] buffer, int offset, int length)
			...
			int len = Input.this.read(buffer, offset, length);
			if (len < 0)
				_isClosed = true;
	...
	final static class ObjectDefinition {
		...
		ObjectDefinition(String type, Deserializer reader, Object[] fields,
                         String[] fieldNames) {
			_type = type;
			_reader = reader;
			_fields = fields;
