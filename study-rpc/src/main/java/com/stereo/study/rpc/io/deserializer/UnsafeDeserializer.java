...
import com.stereo.study.rpc.exc.FieldException;
...
public class UnsafeDeserializer extends AbstractMapDeserializer {
	private static final Logger log = Logger.getLogger(JavaDeserializer.class
			.getName());
	...
	public Object readMap(AbstractInput in, Object obj) throws IOException {
		try {
			...
			while (!in.isEnd()) {
				...
				FieldDeserializer deser = (FieldDeserializer) _fieldMap
						.get(key);
	...
	public Object readObject(AbstractInput in, Object obj,
                             FieldDeserializer[] fields) throws IOException {
		try {
			int ref = in.addRef(obj);
	...
	public Object readObject(AbstractInput in, Object obj, String[] fieldNames)
			throws IOException {
		try {
			int ref = in.addRef(obj);
	...
	protected Object resolve(AbstractInput in, Object obj) throws Exception {
	...
	abstract static class FieldDeserializer {
		abstract void deserialize(AbstractInput in, Object obj)
				throws IOException;
	}
	...
	static class NullFieldDeserializer extends FieldDeserializer {
		...
		@Override
		void deserialize(AbstractInput in, Object obj) throws IOException {
			in.readObject();
		}
	}
	...
	static void logDeserializeError(Field field, Object obj, Object value,
		...
		String fieldName = (field.getDeclaringClass().getName() + "." + field
				.getName());

		if (e instanceof FieldException)
			throw (FieldException) e;
		else if (e instanceof IOException)
			throw new FieldException(fieldName + ": " + e.getMessage(), e);
