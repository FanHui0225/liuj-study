...
public class FileUtil {
	 ...
	 * @param folder
	 */
	public static void moveFileToFolder(String oldPath, String folder) {
		File file = new File(oldPath);
		String filename = oldPath.substring(oldPath.lastIndexOf(File.separator));
		if (file.exists()) {
	 ...
	 * @param folder
	 */
	public static void moveDeepFileToFolder(String oldPath, String folder) {
		String filename = oldPath.substring(oldPath.lastIndexOf(File.separator));
		moveFile(oldPath, folder + filename);
	}
	 ...
	 * @return
	 */
	public static List<File> getAllFileList(File folder) {
		...
		List<File> allfiles = new ArrayList<File>(Arrays.asList(files));
		File[] directorys = folder.listFiles(new FileFilter() {
			@Override
			public boolean accept(File pathname) {
		...
		for (File f : directorys)
			allfiles.addAll(getAllFileList(f));
		return allfiles;
	}
	 ...
	 * @return
	 */
	public static String getName(String filesrc) {
		int index = filesrc.lastIndexOf(File.separator);
		return filesrc.substring(index + 1);
	}
	 ...
	 * @return
	 */
	public static String getNameNoExt(String filesrc) {
		if (filesrc.lastIndexOf(File.separator) > 0)
			filesrc = getName(filesrc);
		if (filesrc.indexOf(".") > -1)
			return filesrc.substring(0, filesrc.lastIndexOf("."));
		else
			return filesrc;
	}

	public static void main(String args[]) {
		// moveFileToFolder("/home/stereo/视频/Hadoop中小规模集群的并行计算方案.pdf",
		// "/home/stereo/视频/demo");
		System.out.println(StringUtils.isEmpty(getFileExtension("/home/stereo/视频/Hadoop中小规模集群的并行)")));
